...
struct MessageContext {
	template <class Derived> struct I : Extension<Required> {
		...
		static const void* extension_data(const char* uri) {
			...
			if (!strcmp(uri, LV2_CONTEXTS_URI "#messageContext")) {
	...
	protected:
		static uint32_t _message_run(LV2_Handle  instance,
		                             ...
		                             void*       valid_outputs) {
			return reinterpret_cast<Derived*>(instance)->message_run(
...
/** Request Run Feature.
 * Plugin will have a host provided method:
 * <code>uint32_t request_run(uint32_t context);</code>
 */
template <bool Required>
struct RequestRunFeature {
	template <class Derived> struct I : Extension<Required> {
		I() : m_request_run_feature(0) {}

		static void map_feature_handlers(FeatureHandlers& hmap) {
			hmap[LV2_CONTEXTS_URI "#RequestRunFeature"] = &I<Derived>::handle_feature;
		}

		static void handle_feature(void* instance, void* data) {
			Derived* d = reinterpret_cast<Derived*>(instance);
			I<Derived>* fe = static_cast<I<Derived>*>(d);
			fe->m_request_run_feature = reinterpret_cast<LV2_Contexts_Request_Run_Feature*>(data);
			fe->m_ok = true;
		}

	protected:
		void request_run(uint32_t context_uri) {
			m_request_run_feature->request_run(
				m_request_run_feature->data,
				context_uri);
		}

		LV2_Contexts_Request_Run_Feature* m_request_run_feature;
	};
};
