...
MainWindow::MainWindow()
	: xml(UIFile::open("eugene_tsp_gui"))
	...
	, _rng(0)
{
	...
	double crossover_probabilities[] = {
...
{
	while (true) {
		const shared_ptr< eugene::GA<TSP::GeneType> > ga(_ga);
		if (ga)
			ga->iteration();
		else
			break;
...
MainWindow::gtk_on_execute()
{
	...
	case 0:
		s = shared_ptr< Selection<TSP::GeneType> >(
			new RouletteSelection<TSP::GeneType>(*p.get()));
		break;
	case 1:
		s = shared_ptr< Selection<TSP::GeneType> >(
			new RankSelection<TSP::GeneType>(*p.get(), selection_probability));
		break;
	case 2:
		s = shared_ptr< Selection<TSP::GeneType> >(
			new TournamentSelection<TSP::GeneType>(*p.get(), trial_size, selection_probability));
		break;
	}

	TSP::Population pop;
	p->initial_population(_rng, pop, p->gene_size(), population_size);
	_ga = shared_ptr<TSPGA>(new TSPGA(_rng, p, s, _crossover, _mutation, pop,
...
MainWindow::gtk_on_open()
{
	...
	if (result == Gtk::RESPONSE_OK) {
		_problem = std::shared_ptr<TSP>(new TSP(filename));
		viewport->remove();
		_canvas = TSPCanvas::load(_problem);
		viewport->add(_canvas->widget());
		_canvas->widget().show();
	}
}
...
MainWindow::gtk_iteration()
{
	...
	const TSP::GeneType best = tspga->best();
	if (!_best || best != *_best.get()) {
		std::ostringstream fit_ss;
		fit_ss << tspga->problem()->evaluate(best);
		shortest_path_label->set_text(fit_ss.str());
		best_generation_label->set_text(gen_ss.str());

		_canvas->update(best);

		_best = std::shared_ptr<TSP::GeneType>(new TSP::GeneType(best));
	}
